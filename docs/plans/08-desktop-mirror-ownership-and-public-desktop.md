...
3. **不改 `storage_path` 语义**：桌面镜像栅栏的 `storage_path` 仍只有用户桌面一个目录
   ...
   `storage_path`；因此公共桌面项**不属于** Sylva 管理区（`is_managed_path`），右键「删除」走
   Shell 原生动词（回收站）而非 Sylva 直删，符合安全方向。
...
- 它**不会被注册**（不在 `visible` 里）——与资源管理器默认视图一致（Sylva 不显示隐藏桌面文件）；
...
### 6.2 真实走查（debug 构建 + `SYLVA_AUTOSTOP_MS`，**已实测**）
