...
#   过去脚本直接运行 cargo 的产物 target\<profile>\sylva.exe，于是实例在跑时，
#   cargo 链接阶段必然失败：
#       error: failed to remove file `G:\Codes\sylva\target\dev-fast\sylva.exe`
#       Caused by: 拒绝访问。 (os error 5)
#   现在把两者解耦：
#       · cargo 只写 target\<profile>\sylva.exe     —— 构建期没有任何进程占用它；
#       · 真正运行的实例是 target\<profile>\sylva-run.exe —— 被占用也不影响构建。
#   流程：cargo build（旧实例在不在跑都能成功）→ 构建成功才优雅停旧实例
#         → 覆盖 sylva-run.exe → 启动；构建失败则原样保留运行中的实例。
#   注意：sylva-run.exe 与 sylva.exe 同目录，因此「<exe 同级>/data」数据目录不变
#         （见 crates/app/src/main.rs:299），栅栏配置与内部库都不会跑丢。
#
# ── 为什么不能简单地「杀掉再启动」（均已实测） ─────────────────────────────
#   1. Sylva 有单实例互斥（`Sylva.Desktop.Fences`，见 crates/app/src/main.rs:258）。
...
#   3. `taskkill /IM sylva.exe`（不带 /F，本意是投递 WM_CLOSE）实测**不可靠**：
...
$exe    = Join-Path $root "target\$Profile\sylva.exe"      # cargo 产物（构建期必须空闲）
$runExe = Join-Path $root "target\$Profile\sylva-run.exe"  # 实际运行的映像（可被占用）
...
# 否则新实例会撞上单实例互斥后静默自退。
function Get-SylvaProcess {
    @(Get-Process -Name 'sylva', 'sylva-run' -ErrorAction SilentlyContinue)
}

# 优雅关闭正在运行的实例，并等待其完全退出。返回 $true 表示可以安全接管。
function Stop-Sylva {
    if ((Get-SylvaProcess).Count -eq 0) { return $true }

    $hwnd = [SylvaWin]::Overlay()
    if ($hwnd -eq [IntPtr]::Zero) {
        Write-Host '==> 警告：实例在运行，但找不到 SylvaOverlay 窗口，无法安全关闭。'
        Write-Host '    请手动按 Ctrl+Shift+F10 退出后再保存文件。本次不重启。'
        return $false
    }

    Write-Host '==> 优雅关闭旧实例（WM_APP_QUIT，保证恢复桌面图标）'
    [void][SylvaWin]::PostMessageW($hwnd, $WM_APP_QUIT, [IntPtr]::Zero, [IntPtr]::Zero)

    $deadline = (Get-Date).AddSeconds($ExitWaitSec)
    while ((Get-SylvaProcess).Count -gt 0 -and (Get-Date) -lt $deadline) {
        Start-Sleep -Milliseconds 100
    }

    if ((Get-SylvaProcess).Count -gt 0) {
...
function Build-And-Run {
    ...
    if (-not (Stop-Sylva)) { return }

    # 旧实例已退出，sylva-run.exe 不再被占用，可以安全覆盖。
    try {
        Copy-Item -LiteralPath $exe -Destination $runExe -Force
    } catch {
        Write-Host "==> 无法覆盖 $runExe：$($_.Exception.Message)"
        Write-Host "    新产物已就绪（target\$Profile\sylva.exe），可手动运行它或重新执行本脚本。"
        return
    }

    Start-Process -FilePath $runExe -WorkingDirectory (Split-Path -Parent $runExe) | Out-Null
    Write-Host "==> 已启动 sylva-run.exe（构建 $([math]::Round($sw.Elapsed.TotalSeconds,1))s）"
}
