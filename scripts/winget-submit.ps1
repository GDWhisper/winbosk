# Publish / update WinBosk on winget: create a branch in the user's fork of
...
param(
    ...
    [string]$Branch = '',
    [string]$Fork = 'GDWhisper/winget-pkgs'
)
$ErrorActionPreference = 'Stop'
if (-not $Branch) { $Branch = "WinBosk.WinBosk-$Version" }
$root = Split-Path -Parent $PSScriptRoot
$dir = Join-Path $root "packaging\winget\manifests\s\WinBosk\WinBosk\$Version"
...
foreach ($f in $files) {
    ...
    $path = "manifests/s/WinBosk/WinBosk/$Version/$($f.Name)"
    $out = gh api "repos/$Fork/contents/$path" -f message="Add WinBosk v$Version" -f branch=$Branch -f content=$content -X PUT --jq '.content.name' 2>&1
    if ($LASTEXITCODE -ne 0) { throw "failed to upload $($f.Name): $out" }
    Write-Host "uploaded: $out"
}

Write-Host "==> open PR"
$shaLine = (Get-Content (Join-Path $dir 'WinBosk.WinBosk.installer.yaml') -Encoding UTF8 | Where-Object { $_ -match '^\s*InstallerSha256:' } | Select-Object -First 1)
$sha = ($shaLine -split ':', 2)[1].Trim()
$body = @"
New version: WinBosk.WinBosk version $Version

Portable zip (x64) hosted on the official release page:
https://github.com/GDWhisper/winbosk/releases/download/v$Version/WinBosk-v$Version-win64.zip

SHA-256: $sha
"@
$pr = gh pr create --repo microsoft/winget-pkgs --head "$Fork`:$Branch" --base $base `
    --title "New version: WinBosk.WinBosk version $Version" --body $body 2>&1
